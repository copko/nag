...
if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){
                    $username_err = "Používateľ s takýmto loginom už existuje.";
                } else{
                    $username = trim($_POST["username"]);
                }
            } else{
                echo "Ups, niečo sa nepodarilo. Skúste to znova neskôr.";
            }
    ...
    if(empty(trim($_POST["confirm_password"]))){
        ...
        if(empty($password_err) && ($password != $confirm_password)){
            $confirm_password_err = "Heslá nesúhlasia.";
        }
    }
...
<body>
    <div class="wrapper">
        <h2>Regisstrácia</h2>
        <p>Prosím, vyplňte tieto polia pre vytvorenie účtu.</p>
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <div class="form-group">
                ...
                <input type="reset" class="btn btn-secondary ml-2" value="Vymazať">
            </div>
